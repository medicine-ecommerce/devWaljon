...
class Vendor_model extends MY_model
{
	...
	{
		$this->db->select('product.*,product.id as product_id,product_item.*');
		$this->db->from('product');
		...
		if ($query->num_rows() > 0) {
			...
			$this->db->select('product_images.id as product_image_id,product_images.image');
			$this->db->from('product_images');
			$this->db->where('product_images.product_id',$id);		
			$query1 = $this->db->get();
			$result->product_images= $query1->result();		
			return $result;
		}
	}
