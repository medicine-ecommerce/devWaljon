...
<script type="text/javascript">
  ...
  $('#import_form').on('submit', function(event){
    ...
    $.ajax({
      url:"<?php echo base_url() ?>admin/upload_manufacturer_bulk",
      method:"POST",
      ...
      success:function(data){                
        if(data.status){          
          $('#file').val('');
            if(user_type=='admin'){
              window.location.href = "<?php echo base_url() ?>admin/manufacturer_list";        
            }else{
              window.location.href = "<?php echo base_url() ?>vendor/manufacturer_list";        
            }
        }else{
          alert('Manufacturer Name Alreay Exist');
