...
<script type="text/javascript">
  ...
  $('#import_form').on('submit', function(event){
    ...
    $.ajax({
      url:"<?php echo base_url() ?>admin/upload_saltComposition_bulk",
      method:"POST",
      ...
      success:function(data){                
        if(data.status){          
          $('#file').val('');
          if(user_type=='admin'){
              window.location.href = "<?php echo base_url() ?>admin/saltComposition_list";        
            }else{
              window.location.href = "<?php echo base_url() ?>vendor/saltComposition_list/<?php echo $this->session->userdata('user_id'); ?>";        
            }
